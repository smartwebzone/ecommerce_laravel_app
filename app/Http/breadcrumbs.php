...
Breadcrumbs::register('blog.post.show', function ($breadcrumbs, $article) {
    ...
    $breadcrumbs->push($article->title, route('dashboard.article.show', [$article->id, $article->slug]));
});
...
Breadcrumbs::register('news.show', function ($breadcrumbs, $news) {
    ...
    $breadcrumbs->push($news->title, route('dashboard.news.show', [$news->id, $news->slug]));
});

Breadcrumbs::register('photo_gallery.show', function ($breadcrumbs, $photo_gallery) {
    $breadcrumbs->parent('home');
    $breadcrumbs->push($photo_gallery->title, route('dashboard.photo_gallery.show', [$photo_gallery->id, $photo_gallery->slug]));
});
