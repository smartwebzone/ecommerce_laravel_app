...
abstract class BaseSample
{
  ...
  public function __construct () {
    ...
    if ($options && array_key_exists('noconfig', $options)) {
      $this->config = [];
    } else {
      if (!$options || !array_key_exists('config_path', $options)) {
        $options['config_path'] = public_path('/google');
      }
  ...
  protected function authenticate(Google_Client $client) {
    try {
      // Try loading the credentials.
        //dd(Google_Service_ShoppingContent::CONTENT);
